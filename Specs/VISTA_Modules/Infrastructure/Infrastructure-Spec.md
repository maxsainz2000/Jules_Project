...
### Requirements
- **[SaleCompletedAccountingHandler.vb](file:///c:/Users/maxsa/Documents/24_7_Agent/VISTA_WPF_Original/Handlers/SaleCompletedAccountingHandler.vb) **: Imported `MerchSys.SharedKernel.Persistence`, injected `ISyncableRepository(Of AccountingDbContext)`, and replaced direct save with `_repository.SaveChangesWithJournalAsync(cancellationToken)`.
- **[GoodsReceivedAccountingHandler.vb](file:///c:/Users/maxsa/Documents/24_7_Agent/VISTA_WPF_Original/Handlers/GoodsReceivedAccountingHandler.vb) **: Imported `MerchSys.SharedKernel.Persistence`, injected `ISyncableRepository(Of AccountingDbContext)`, and replaced direct save with `_repository.SaveChangesWithJournalAsync(cancellationToken)`.
- **[CreditPaymentAccountingHandler.vb](file:///c:/Users/maxsa/Documents/24_7_Agent/VISTA_WPF_Original/Handlers/CreditPaymentAccountingHandler.vb) **: Imported `MerchSys.SharedKernel.Persistence`, injected `ISyncableRepository(Of AccountingDbContext)`, and replaced direct save with `_repository.SaveChangesWithJournalAsync(cancellationToken)`.
- **[ShrinkageAccountingHandler.vb](file:///c:/Users/maxsa/Documents/24_7_Agent/VISTA_WPF_Original/Handlers/ShrinkageAccountingHandler.vb) **: Imported `MerchSys.SharedKernel.Persistence`, injected `ISyncableRepository(Of AccountingDbContext)`, and replaced direct save with `_repository.SaveChangesWithJournalAsync(cancellationToken)`.
- **[SaleCompletedWithVatHandler.vb](file:///c:/Users/maxsa/Documents/24_7_Agent/VISTA_WPF_Original/Handlers/SaleCompletedWithVatHandler.vb) **: Imported `MerchSys.SharedKernel.Persistence`, injected `ISyncableRepository(Of AccountingDbContext)`, and replaced direct save with `_repository.SaveChangesWithJournalAsync(cancellationToken)`.
- **[GoodsReceivedWithVatHandler.vb](file:///c:/Users/maxsa/Documents/24_7_Agent/VISTA_WPF_Original/Handlers/GoodsReceivedWithVatHandler.vb) **: Imported `MerchSys.SharedKernel.Persistence`, injected `ISyncableRepository(Of AccountingDbContext)`, and replaced direct save with `_repository.SaveChangesWithJournalAsync(cancellationToken)`.
- Verified [ReceiptTamperDetectedHandler.vb](file:///c:/Users/maxsa/Documents/24_7_Agent/VISTA_WPF_Original/Handlers/ReceiptTamperDetectedHandler.vb) remains unchanged, utilizing direct context saving since its audit log records are local-only and decorated `<NoSync>`.
...
### Requirements
- **[mariadb-inv-salecogs-schema.sql](file:///c:/Users/maxsa/Documents/24_7_Agent/VISTA_WPF_Original/Plans/VISTA_Modules/Infrastructure/sql/mariadb-inv-salecogs-schema.sql) **: central table DDL script.
- **[AddInvSaleCogs.sql](file:///c:/Users/maxsa/Documents/24_7_Agent/VISTA_WPF_Original/Data/Migrations/Central/AddInvSaleCogs.sql) **: migration script for existing deployments.
- **[mariadb-init.sql](file:///c:/Users/maxsa/Documents/24_7_Agent/VISTA_WPF_Original/Plans/VISTA_Modules/Infrastructure/sql/mariadb-init.sql) **: added central schema table creation for fresh deploys.
- **[InventorySyncMap.vb](file:///c:/Users/maxsa/Documents/24_7_Agent/VISTA_WPF_Original/Sync/SyncMaps/InventorySyncMap.vb) **: added `RemoteSaleCogs` POCO, `"Inv_SaleCogs"` to the tables registry list, and its payload deserialization in `ToRemote`.
- **[MariaDbSyncContext.vb](file:///c:/Users/maxsa/Documents/24_7_Agent/VISTA_WPF_Original/Sync/MariaDbSyncContext.vb) **: updated `FetchRemoteRowAsync` to dynamically check the table name and select `DeductedAt` instead of `ModifiedAt` for `Inv_SaleCogs`, ensuring conflict resolution checks succeed without schema mismatches.
...
- Created `MariaDbSchemaInitializer.vb` in `Data/` — Executed embedded DDL resources inside a transaction using custom statement splitting to support `DELIMITER` blocks; hashes each applied script to ensure SHA-256 matching. Seeds default system user accounts (manager, owner) using direct PasswordHashHelper encryption. **CRITICAL QA CONSTRAINT**: It MUST ALSO seed the Developer account (`EnsureDeveloperAccount`) as required by project architecture rules. Additionally, this data-layer class must NOT contain `MessageBox.Show()` calls. It must throw exceptions to be caught by the caller.
- **Program.vb** (or equivalent WinForms startup): Replaced legacy SQLite initializer call with `MariaDbSchemaInitializer.Initialize` inside startup blocks, passing central DB connection and logger dependencies. **CRITICAL QA CONSTRAINT**: The try-catch block and `MessageBox.Show()` abort UI must be located here in the presentation layer, not inside the initializer.
